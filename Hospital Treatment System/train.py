...
FINE_TUNE_AT = 80     # 解冻 MobileNetV2 最后 80 层（约前 80% 冻结，20% 解冻）
FINE_TUNE_LR = 1e-5   # 微调学习率（比初始低 10 倍）
...
# ── 数据准备 ──────────────────────────────────────────
data_root = pathlib.Path("./train_photos")
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dropout(0.3),              # 防过拟合
...
# ── 可视化 ──────────────────────────────────────────────
def plot_history(h1, h2, output_path="model.png"):
